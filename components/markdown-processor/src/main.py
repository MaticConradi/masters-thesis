...
def process_file(filename: str):
	...
	try:
		...
		response = client.responses.create(
			model="gpt-5-nano",
			instructions="Combine the two provided texts, ensuring that the formatting is preserved and the content is accurate. The output should be in markdown format. The output text should not contain placeholders or references to the original content; always provide the full text, including equations, figure descriptions, tables, references, headers and paragraphs. The output text should be wrapped in a code block marked with ``` at the start and at the end of the text. Do not add any additional formatting beyond what is necessary to preserve the original structure and content.\n\nNotes:\n- Unformatted text is not going to contain correct equations, so it is important to use the markdown text as the primary source.\n- Always use the same header structure and level as in the markdown text, and do not change the order of sections.\n- Retain the table format used (likely latex) in the markdown text.\n- Do not summarize or alter the content in any way, just ensure that the markdown formatting is applied correctly and any missing or incorrect text is filled in accurately.",
